...
                    error: err,
                    code: 1,
                    message: `Failed to execute action: ${Action} on server`,
                })
            }
            return res.json({
                status: "Success",
                error: null,
                code: 0,
                message: `Successfully executed action: ${execAction}, Please Restart PJBX to effect changes`,
                info: execAction,
            })
        })
...
    const userFileObj = JSON.parse(fs.readFileSync('./PJBX/config/user.json'))
...
    const hasErrors = !vResult.isEmpty();
    if (hasErrors) {
            return.status(402).json(vResult.errors);
...
    infoObj.ANTIBOT = userFileObj.GATE_KEY ? 'ON' : ''
...
    const bashExec = exec('bash scripts/reboot-PJBX.sh', function(err, stdout, stderr) {
        if (err) {
            console.error("Early Failure for Reboot work...")
            return res.json({
                status: "Error",
                error: "Failed To Reboot and Update the PJBX Instance ",
                code: code,
                needRestart: false,
                message: 'Failed To Reboot and Update the PJBX Instance, Check if your VPS is active, or try doing from terminal with command "npm run restart"'
            })
        }
    });
